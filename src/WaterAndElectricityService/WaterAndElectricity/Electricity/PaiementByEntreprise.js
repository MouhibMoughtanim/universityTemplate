...
export default function PaiementByEntreprise() {
    ...
    const loadFacts = async () => {
        const result = await axios.get(ipURL+`:8081/factElec/factElecs/entreprise=${id}`);
        setFacts(result.data);

    }
    const loadFactsP = async () => {
        const result = await axios.get(ipURL+`:8081/factElec/factElecsP/entreprise=${id}`);
        setFactsP(result.data);

    }

    const loadFactsNP = async () => {
        const result = await axios.get(ipURL+`:8081/factElec/factElecsNP/entreprise=${id}`);
        setFactsNP(result.data);

    }

    const loadFactsRef = async (refAll) => {
        const result = await axios.get(ipURL+`:8081/factElec/factElecs/entreprise=${id}/ref=${refAll}`)
        setFacts(result.data)

    }

    const loadFactsRefP = async (refAllP) => {
        const result = await axios.get(ipURL+`:8081/factElec/factElecsP/entreprise=${id}/ref=${refAllP}`)
        setFactsP(result.data)

    }

    const loadFactsRefNP = async (refAllNP) => {
        const result = await axios.get(ipURL+`:8081/factElec/factElecsNP/entreprise=${id}/ref=${refAllNP}`)
        setFactsNP(result.data)

    }
    const payerFacture = async (id) => {
        await axios.put(ipURL+`:8081/factElec/paye/id=${id}`)
        loadFacts();
        loadFactsP();
        loadFactsNP();
    }
