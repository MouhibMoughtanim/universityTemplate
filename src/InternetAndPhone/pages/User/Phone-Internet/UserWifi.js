...
function UserWifi() {
  ...
  const loadWifis = async()=>{
    const all = await axios.get('http://localhost:2222/Phone-Internet/wifi/not')
    setWifis(all.data)
...
const updateWifi = async(e) => {
  ...
  console.log(wifiz)
  const wifi = await axios.post('http://localhost:2222/Phone-Internet/wifi/update', wifiz)
...
const search = async(e)=>{
  ...
  const some = await axios.get('http://localhost:2222/Phone-Internet/wifi/not-code/'+sid)
  setWifis(some.data)
