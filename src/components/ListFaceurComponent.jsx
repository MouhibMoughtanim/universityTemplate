...
class ListFaceurComponent extends Component {
    constructor(props){
     
        super(props)
        this.state={

            entroprise_id:this.props.match.params.id,
      ...
      Swal.fire({
       ...
       this.sendEmail()
        ...
        CarService.updateCarEtat(car_id,car).then(res=>{
          this.props.history.push(`/pCar/${car_id}`);
          window.location.reload(true);
      })
      },2000)
    }
      ...
      Swal.fire({
        ...
        setTimeout(()=>{ 
          HouseService.updateHouseEtat(house_id,house).then(res=>{
                  this.props.history.push(`/p/${house_id}`);
                  window.location.reload(true);
        })
      },2000)
    ...
    updateCar=(h)=>{
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            Swal.fire(
              'Bill!',
              'Your bill has been payed.',
              'success',
              CarService.updateCarEtat(this.state.billId,car).then(res=>{
                this.props.history.push(`/factory_entrobrise/${this.state.entroprise_id}`);
                window.location.reload(true);
            })
            )
          }
...
export default ListFaceurComponent;