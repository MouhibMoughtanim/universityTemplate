...
export default function Factures() {
  ...
  useEffect(() => {
    fetch(ipURL+`:8082/fournisseur/getF/${pathVariable}`, {
      headers: {
  ...
  useEffect(() => {
    fetch(ipURL+`:8082/facture/getFactureByFournisseur/${pathVariable}`, {
      headers: {
        'Content-Type': 'application/json'
      }
    })
  ...
  const deletedata = (itemToDelete) => {
    axios.delete(process.env.ilyas+`facture/delete/${itemToDelete.id_fournisseur}`)
      .then(res => {
  ...
  const handleSubmit = (event) => {
    ...
    fetch(process.env.ilyas+`facture/add`, {
      method: 'POST',
